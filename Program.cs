...
namespace PsVDecrypt
{
    public class Program
    {
        ...
        private static void DecryptCourse(string courseSrcDir)
        {
            ...
            var realCourseName = String.Concat(((String)dataTable.Rows[0]["Title"]).Where(x=> !invalidFileChars.Contains(x)));
            var courseDstDir = Path.Combine(OutputDir, realCourseName);
            Console.WriteLine("Processing course " + realCourseName + " ..");
            ...
            try
            {
                ...
                for (var i = 0; i < dataTable.Rows.Count; i++)
                {
                    ...
                    for (var j = 0; j < clipsDataTable.Rows.Count; j++)
                    {
                        ...
                        var videoName = Path.Combine(moduleDstDir,
                                          (clipItem["ClipIndex"].ToString()).PadLeft(2, '0') + "." +
                                          Util.TitleToFileName((string)clipItem["Title"]));

                        if(videoName.Length + 4 > 255)
                        {
                            videoName = videoName.Substring(0, 251);
                        }

                        var clipDst = videoName + ".mp4";
